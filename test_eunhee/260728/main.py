...
from paper_edge_mapper import PaperEdgeMapper
...
def open_camera(index, name, width=WIN_W, height=WIN_H):
    ...
    # DirectShow 등은 요청한 해상도를 지원하지 않으면 조용히 다른 값으로 낮출 수 있어서,
    # 실제로 적용된 해상도를 눈으로 확인할 수 있게 출력합니다.
    actual_w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    actual_h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    print(f"[카메라] {name}: 요청 {width}x{height} -> 실제 적용 {actual_w}x{actual_h}")
    return cap
...
def main():
    ...
    # 레이저 카메라는 종이 검출 정밀도를 위해 더 높은 해상도로 캡처합니다.
    # 레이저 검출/화면 표시는 여전히 WIN_W x WIN_H(640x480)로 리사이즈해서 사용하므로
    # 레이저 픽셀 임계값 등 기존 튜닝값에는 영향이 없습니다.
    laser_cap = open_camera(
        LASER_CAMERA_INDEX, "레이저", width=LASER_CAPTURE_W, height=LASER_CAPTURE_H
    )
    ...
    if USE_PAPER_MARKER_BOARD:
        if str(PAPER_MAPPER_MODE).lower() == "edge":
            board_mapper = PaperEdgeMapper()
        else:
            board_mapper = PaperBoardMapper()
    else:
        board_mapper = None
    ...
    try:
        while True:
            ...
            # ==================================================
            ret_laser, raw_frame = laser_cap.read()
            if not ret_laser or raw_frame is None or raw_frame.size == 0:
                raw_frame = np.zeros((WIN_H, WIN_W, 3), dtype=np.uint8)

            # 레이저 검출/화면 표시는 계속 640x480 기준(frame)을 씁니다.
            frame = cv2.resize(raw_frame, (WIN_W, WIN_H))

            board_ready = True
            if board_mapper is not None:
                # 종이 사각형 검출만 고해상도 raw_frame 으로 수행하고, 좌표는 board_mapper
                # 내부에서 frame(640x480) 기준으로 환산합니다.
                raw_board_ready = board_mapper.update(frame, detect_frame=raw_frame)
